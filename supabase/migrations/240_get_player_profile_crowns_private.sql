-- 240_get_player_profile_crowns_private.sql
-- WHY : confidentialité du solde de Couronnes. Le front masquait déjà l'affichage
-- sur le profil d'autrui (V0.9.52), mais la RPC renvoyait toujours crownsBalance
-- dans le payload — inspectable côté réseau (revue sécu auto, fuite IDOR-style).
-- On gate le champ côté serveur : crownsBalance n'est renvoyé QUE si l'appelant
-- consulte son propre profil (p_user_id = auth.uid()::text, le mapping identité
-- canonique du schéma — cf. mig 190/194), sinon NULL. On ne doit pas pouvoir
-- jauger la richesse d'autrui, et ce n'est plus contournable.
-- Discipline B1 : copie verbatim de la baseline (mig 207), seul change le CASE
-- sur 'crownsBalance' dans le json_build_object final.
...
DECLARE
  ...
  v_xp_for_current_level INT;  -- V158 (10/05) : pour fix progress bar UI
...
BEGIN
  ...
  v_xp_for_current_level := public._xp_for_level(v_level);  -- V158
  ...
  SELECT json_build_object(
    ...
    -- Couronnes privées (V0.9.53, mig 240) : renvoyées UNIQUEMENT si l'appelant
    -- consulte son propre profil. Sinon NULL → non inspectable côté réseau.
