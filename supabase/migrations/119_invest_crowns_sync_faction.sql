-- 119_invest_crowns_sync_faction.sql
-- WHY : bug constaté en prod (Ruines des arènes de Cimier, 7 mai 2026).
-- Quand un lieu bascule par influence (Couronnes), `invest_crowns` met à
-- jour `place_veille.expedition_id` mais OUBLIE de mettre à jour
-- `place_veille.faction_id` et `is_neutral`. Conséquence : la carte garde
-- la couleur de l'ancien veilleur (Gautier), même si Uriel est devenu
-- veilleur par Couronnes. Visuellement faux, gameplay confus.
--
-- Le cas vacant (lignes 195-208 de mig 097) écrit bien faction_id à
-- l'INSERT car c'est une création. Le bug est dans le cas "lieu déjà
-- veillé" (lignes 209-234) : seul un UPDATE partiel.
--
-- FIX : reprise verbatim de mig 097 (procédure XO "lire avant de
-- réécrire") — seuls 4 changements ciblés :
--   1) v_target_faction/v_target_neutral remontés AVANT la branche
--      attack pour être disponibles dans les deux cas (vacant +
--      veillé).
--   2) UPDATE place_veille (cas veillé) : ajout de faction_id et
--      is_neutral.
--   3) (Tout le reste reste verbatim — plant_flag inclus, on ne
--      touche pas à plant_flag.)
--
-- BACKFILL : à la fin, on resync toutes les place_veille existantes
-- prises par influence dont la faction_id ne matche pas leur expé
-- courante. Restaure la cohérence des couleurs sur la carte pour
-- les lieux bugués passés (cf. Cimier).

BEGIN;

-- ============================================================
-- invest_crowns — V119 : sync faction_id sur bascule par influence
-- Reprise verbatim mig 097, modifs en commentaire ★FIX119★.
-- ============================================================
...
BEGIN
  ...
  -- ★FIX119★ — récupérer la faction de l'expédition cible UNE FOIS,
  -- avant la branche bascule. Comme ça les deux chemins (vacant + veillé)
  -- y ont accès. Avant V119, le cas veillé ne lisait pas v_target_faction
  -- et le UPDATE place_veille oubliait faction_id/is_neutral.
  ...
  IF v_side = 'attack' THEN
    ...
    ELSE
      ...
      IF v_new_target_score > v_veilleur_score THEN
        ...
        -- ★FIX119★ — ajout faction_id + is_neutral, sans quoi la carte
        -- garde la couleur de l'ancien veilleur après bascule par influence.
...
-- ============================================================
-- BACKFILL — resync place_veille avec la bonne faction pour
-- les lieux pris par influence avant le fix V119.
-- ============================================================

UPDATE public.place_veille pv
SET faction_id = e.faction_id,
    is_neutral = COALESCE(e.is_neutral, false)
FROM public.expeditions e
WHERE pv.expedition_id = e.id
  AND pv.by_influence = true
  AND (pv.faction_id IS DISTINCT FROM e.faction_id
       OR pv.is_neutral IS DISTINCT FROM COALESCE(e.is_neutral, false));
