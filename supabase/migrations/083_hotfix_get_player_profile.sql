-- 083_hotfix_get_player_profile.sql
-- WHY : HOTFIX URGENT. La mig 077 a réécrit get_player_profile à partir de
-- la version mig 032, écrasant les évolutions 045/051/067 (ajout level,
-- xpTotal, xpToNextLevel, xpForNextLevel, veteranFirstEra, crownsBalance,
-- coupeScoreCurrentSeason, coupeSeasonName, et formules associées).
--
-- Conséquence en prod (5 mai 2026 nuit) :
--   - Tous les joueurs apparaissent niveau 1 (level absent du retour JSON)
--   - Plus de balance Couronnes lue côté frontend
--   - Plus de score Coupe saison
--   - Cascade : usePlayer ne s'hydrate pas → carte vide
--
-- ROOT CAUSE : règle B1 violée — j'ai cherché par grep "influence_stock"
-- au lieu de chercher la dernière version réelle de get_player_profile dans
-- le repo. Mig 032 avait influenceStock dans son JSON (donc apparue dans le
-- grep), mais mig 067 (postérieure, version réelle prod) ne l'avait plus —
-- la régression de la 067 → 032 a perdu tout le reste.
--
-- FIX : RESTAURER VERBATIM la version mig 067 de get_player_profile.
-- Aucune modification — la mig 067 ne référence pas users.influence_stock,
-- donc compatible avec la colonne droppée par mig 077.
...
BEGIN
  ...
  -- Score Coupe saison courante (helper centralisé V067)
