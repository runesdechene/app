-- 302_founding_investment_full_weight.sql
-- WHY : la mig 299 pondérait TOUTES les couronnes ÷10 dans le classement des membres (Hall)
-- et le Chef. Résultat : un fondateur ayant investi 200 🪙 à la fondation ne pesait que 20 pts
-- → dépassé en une journée. Décision (Uriel, 25/06) : l'INVESTISSEMENT DE FONDATION
-- (`crowns_invested`) compte PLEIN POT (1:1, ex. 200 = 200 pts ⭐, « points de fondation »),
-- tandis que les couronnes CONQUISES (`crowns_conquered`) restent ÷10. Seule la pondération
-- du tri change (mérite via banner-history inchangé). ADDITIF (2 redéfinitions).

-- ── Chef : mérite (cette Compagnie) + fondation (plein) + conquis (÷10) ──
...
-- ── get_faction_detail : tri = mérite + fondation (plein) + conquis (÷10) ──
-- (corps = mig 299 + isAlly, seul l'ORDER BY change.)
