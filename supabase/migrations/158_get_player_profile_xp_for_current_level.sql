-- 158_get_player_profile_xp_for_current_level.sql
-- WHY : la GloryProgressBar côté frontend calculait progressPct =
-- (xpForNextLevel - xpToNextLevel) / xpForNextLevel, soit le ratio "xp totale
-- depuis niv 1 sur cumul jusqu'à niv+1" — pas la progression dans le niveau
-- courant. Pour un user niv 30 (1926 xp seuil, 2057 niv+1), la barre affichait
-- 93% au lieu de 0.7%. Pour fixer proprement côté frontend, on a besoin du
-- seuil bas (xp_for_level du niveau courant) afin de calculer
-- (xpTotal - xpForCurrentLevel) / (xpForNextLevel - xpForCurrentLevel).
--
-- Cette mig étend get_player_profile pour exposer xpForCurrentLevel.
-- Discipline B1 : copie verbatim de la baseline + 3 ajouts ciblés
-- (DECLARE, calcul, json output).
