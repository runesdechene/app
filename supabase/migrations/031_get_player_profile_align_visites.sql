-- 031_get_player_profile_align_visites.sql
-- WHY :
--   1. Bug "48 lieux explorés vs 44 visités" — get_player_profile lisait
--      `places_explored` (table fantôme legacy, plus alimentée par les RPCs
--      récentes) pour `discoveredPlaces`, alors que `lieuxExplores` (mig 030)
--      lit la table active `place_explorers`. Deux sources, deux comptes.
--      Fix : aligner discoveredPlaces sur place_explorers.visited_at.
--   2. Ajout d'un nouveau champ `lieuxVeilles` : nombre de lieux ACTUELLEMENT
--      veillés par le user (count DISTINCT pv.place_id JOIN expedition_members).
--      Cohérent avec le tab "Lieux veillés" du Leaderboard (mig 027).
--
-- Reprise EXACTE de la mig 030, seuls 2 changements :
--   - bloc v_discovered_places : `places_explored pe`+`pe.created_at`
--     → `place_explorers pe`+`pe.visited_at`
--   - ajout variable v_lieux_veilles + champ 'lieuxVeilles' dans json_build
...
BEGIN
  ...
  -- V0.7 phase 3.5 — fix : on lit place_explorers (table active) au lieu de
  -- places_explored (table fantôme legacy). Cohérence avec lieuxExplores.
  ...
  -- Un user supplanté ne compte plus son ancien plantage.
