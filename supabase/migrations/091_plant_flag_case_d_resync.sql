-- 091_plant_flag_case_d_resync.sql
-- WHY : protéger les investissements défense des veilleurs actifs.
--
-- Avant cette mig (mig 081), un veilleur qui replantait sur SON propre lieu
-- (cas C standard) :
--   1. Devait attendre 24h (cooldown anti-farm)
--   2. Créait une NOUVELLE expédition (nouvel UUID) qui supplantait l'ancienne
--   3. Triggerait un DELETE FROM place_court_score WHERE place_id = X →
--      perdait toute la défense investie (faveur 80 → faveur 50)
--
-- Cas D (NEW) — Resync IRL par membre de l'expé veilleuse plein-veilleur :
--   État : by_influence = false ET le planteur appartient à l'expé veilleuse actuelle.
--   Action :
--     - UPDATE place_veille SET planted_at = now() (resync)
--     - Pas de nouvelle expé créée
--     - Pas de DELETE court_score → conserve la défense investie
--     - Pas de cooldown 24h (replanter chez soi est une visite valide)
--     - Une ligne veille_history pour la trace
--   Pas de notif spéciale (le toast plant_flag standard suffira si on en log un).
--
-- Reste verbatim mig 081 + insertion du cas D entre B et C.
...
BEGIN
  ...
  -- État actuel pour décider du cas
  ...
  -- ============================================================
  -- CAS D — Resync IRL par membre de l'expé veilleuse plein-veilleur (V091)
  -- État : by_influence = false ET le planteur ∈ l'expé veilleuse actuelle.
  -- Action : simple resync, pas de nouvelle expé, conserve la défense
  -- investie (place_court_score), pas de cooldown.
  -- ============================================================
  ...
  THEN
    ...
    -- Une ligne veille_history pour la trace (pas d'effacement scores)
    INSERT INTO public.veille_history (place_id, expedition_id, user_id, faction_id, is_neutral, planted_at)
    VALUES (p_place_id, v_prev_veilleur, p_user_id, v_user_faction, false, v_now);
  ...
  -- ============================================================
  -- CAS C — Plantage standard (verbatim mig 081)
  -- ============================================================
