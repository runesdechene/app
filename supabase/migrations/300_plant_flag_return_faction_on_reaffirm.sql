-- 300_plant_flag_return_faction_on_reaffirm.sql
-- WHY : bug « réaffirmer mon étendard repasse le lieu en neutre » (rapport joueur Rémy).
-- Côté serveur tout est correct (la veille garde sa faction). Mais le front applique un
-- override OPTIMISTE de couleur après plant_flag :
--     pushVeilleOverride(placeId, result.factionId, result.isNeutral, ...)   (VeilleFrame.tsx)
-- Or les modes reclaim_gps (CAS A), confirm_gps (CAS B) et reaffirm_gps (CAS D) NE renvoyaient
-- PAS factionId/isNeutral → undefined → le lieu s'affiche neutre jusqu'au prochain refresh.
-- Fix à la source : ces 3 modes renvoient désormais le factionId/isNeutral RÉELS (lus dans
-- place_veille après l'update). Corps identique à la prod, seuls les 3 RETURN changent. ADDITIF.
