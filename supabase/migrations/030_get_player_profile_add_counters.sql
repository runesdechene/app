-- 030_get_player_profile_add_counters.sql
-- WHY : ajouter au retour de get_player_profile les 2 nouveaux compteurs V0.7 :
--   - lieuxExplores : COUNT(DISTINCT place_id) sur place_explorers
--   - enigmasSolved : COUNT(*) sur enigma_responses correct=TRUE
--
-- Le profil affiche désormais ces 2 compteurs à la place de l'ancien trio
-- (lieux ajoutés / visités / influencés) qui mélangeait V0.5 et faisait
-- doublon avec d'autres infos déjà visibles (lieux ajoutés est dans la
-- Gloire, influencés n'a plus de sens depuis le freeze V0.5).
--
-- Reprise EXACTE de la mig 028 (procédure "lire avant de réécrire"), seuls
-- 2 nouveaux champs sont ajoutés au json_build_object final + 2 nouvelles
-- variables locales.
...
BEGIN
  ...
  -- V0.7 phase 3.5 — nouveaux compteurs visibles dans le profil
