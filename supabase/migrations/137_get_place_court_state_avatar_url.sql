-- 137_get_place_court_state_avatar_url.sql
-- WHY : la barre de tension Cour V0.7.6 affiche maintenant des avatars en
-- cluster Discord-style à côté de la jauge. Aujourd'hui le json_build_object
-- des topPatrons ne contient PAS l'avatar_url des users — donc le composant
-- tombe sur l'initiale par défaut, même quand l'utilisateur a un avatar.
--
-- Recadrage Uriel 8/05 : "j'ai un avatar sur mon profil, pourquoi tu
-- l'affiches pas". Fix : ajouter u.avatar_url dans les deux blocs topPatrons
-- (cas vacant + cas veillé).
--
-- Reprise verbatim de la mig 135. Modifs ciblées sur les 2 sub-selects qui
-- construisent les topPatrons : ajout d'un 'avatarUrl': u.avatar_url.
