-- 280_coupe_state_member_gated.sql
-- WHY : suite 279. get_coupe_state agrégeait l'action des Compagnies sur la bannière
-- active (user_with_meta) sans vérifier l'appartenance → un straggler à points d'action
-- aurait pollué. On JOIN faction_members dans faction_action (member-gated), cohérent
-- avec list_factions/get_faction_detail. Le gold est déjà member-gated via _faction_gold_coupe.
-- Seule la CTE faction_action change vs 278 ; le reste est identique.
