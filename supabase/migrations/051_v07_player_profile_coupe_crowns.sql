-- 051_v07_player_profile_coupe_crowns.sql
-- WHY : exposer crownsBalance + coupeScoreCurrentSeason + coupeSeasonName
-- dans get_player_profile pour pouvoir les afficher sur les profils des AUTRES
-- joueurs (pas seulement soi via useCrownsStore / useCoupe).
--
-- Sources :
--   - crownsBalance  : public.user_crowns.balance (mig 021)
--   - coupeScore     : même formule que get_coupe_state myBreakdown (mig 039)
--                      = visites*1 + places*7 + carnets*3 + photos*1 +
--                        plantages*5 + _enigma_score_weighted(fenêtre saison)
--   - coupeSeasonName: coupe_seasons.name sur la saison active (ended_at IS NULL)
--
-- Reprise EXACTE de mig 045 (get_player_profile), avec 3 champs supplémentaires.
...
BEGIN
  ...
  -- Filtrage des titres affichés (idem mig 045)
  ...
  -- ============================================================
  -- NEW : Couronnes balance
  -- ============================================================
  SELECT COALESCE(balance, 0) INTO v_crowns_balance
  FROM public.user_crowns
  WHERE user_id = p_user_id;
  v_crowns_balance := COALESCE(v_crowns_balance, 0);

  -- ============================================================
  -- NEW : Score Coupe saison courante
  -- Même formule que get_coupe_state myBreakdown (mig 039) :
  -- visites*1 + places*7 + carnets*3 + photos*1 + plantages*5 + enigmes_pondérés
  -- ============================================================
  ...
  IF v_coupe_season.id IS NOT NULL THEN
    ...
    -- Score énigmes pondéré (1/1/2/3) sur la fenêtre saison
    v_coupe_enigmes_score := public._enigma_score_weighted(p_user_id, v_coupe_season.started_at, v_coupe_window_end);
    ...
    + COALESCE(v_coupe_enigmes_score, 0)
    INTO v_coupe_score_current_season
    FROM
      (SELECT COUNT(DISTINCT place_id)::int AS cnt FROM public.place_explorers
         WHERE user_id = p_user_id
           AND visited_at >= v_coupe_season.started_at AND visited_at < v_coupe_window_end) my_visites,
      (SELECT COUNT(*)::int AS cnt FROM public.places
         WHERE author_id = p_user_id
           AND created_at >= v_coupe_season.started_at AND created_at < v_coupe_window_end) my_places,
      (SELECT COUNT(*)::int AS cnt FROM public.place_contributions
         WHERE user_id = p_user_id AND type = 'carnet'
           AND created_at >= v_coupe_season.started_at AND created_at < v_coupe_window_end) my_carnets,
      (SELECT COALESCE(SUM(
       ...
       WHERE user_id = p_user_id
         AND created_at >= v_coupe_season.started_at AND created_at < v_coupe_window_end) my_photos,
      (SELECT COUNT(*)::int AS cnt FROM public.veille_history
         WHERE user_id = p_user_id
           AND planted_at >= v_coupe_season.started_at AND planted_at < v_coupe_window_end) my_plantages;
  ELSE
  ...
  SELECT json_build_object(
    ...
    -- NEW : Couronnes & Coupe exposés pour tous les profils (mig 051)
