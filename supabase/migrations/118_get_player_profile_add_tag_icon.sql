-- 118_get_player_profile_add_tag_icon.sql
-- WHY : sur le profil joueur, les 3 carrousels (cartographié / planté /
-- visité) n'affichaient que le titre du place_type. Pour donner un
-- repère visuel rapide, on veut afficher l'icône emoji du tag primary
-- (cf. table place_tags is_primary=TRUE → tags.icon) avant le titre,
-- comme sur les markers de la carte.
--
-- Reprise EXACTE de la mig 083 (procédure "lire avant de réécrire") —
-- seuls 4 endroits changent :
--   1) authored : LEFT JOIN place_tags+tags primary + 'tagIcon' dans JSON
--   2) discovered : idem
--   3) favorite : idem (par cohérence, même si pas utilisé en UI active)
--   4) veilled : idem
-- Tout le reste (level, XP, Couronnes, Coupe, displayed titles,
-- COALESCE first_name/email, etc.) est identique octet pour octet.
...
BEGIN
  ...
  -- ── authored : + tagIcon (LEFT JOIN place_tags primary + tags) ──
  ...
  -- ── favorite : + tagIcon (par cohérence) ──
