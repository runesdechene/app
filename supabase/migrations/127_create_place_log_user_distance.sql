-- 127_create_place_log_user_distance.sql
-- WHY : debug du bug "je ne peux pas planter mon étendard" (remonté par
-- Tugdual le 8/05). L'auto-plant côté front échoue silencieusement quand
-- _create_place_internal calcule v_is_gps = FALSE. On ne sait pas pourquoi
-- (position pas envoyée ? loin ? imprécise ?) parce qu'on ne logue rien.
--
-- Cette mig enrichit le data du log activity_log.new_place avec :
--   - userDistanceKm  : distance calculée entre user et lieu (NULL si position
--                       pas fournie)
--   - userPosProvided : boolean — la position user a-t-elle été envoyée
--
-- Reprise B1 verbatim de la mig 126, seul change : le bloc INSERT activity_log
-- du retour qui ajoute les deux nouveaux champs au jsonb.
...
BEGIN
  ...
  IF v_is_gps THEN
    ...
    -- rétrocompat front, plus aucune écriture dans place_influence /
  ...
  -- V127 : userDistanceKm + userPosProvided pour debug du bug auto-plant
