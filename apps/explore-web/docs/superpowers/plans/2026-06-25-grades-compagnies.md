...
| Investissement de fondation dans le classement | **TRANCHÉ (Uriel 25/06)** : `crowns_invested` ne pèse **que pendant la saison de fondation** (= `factions.created_at` dans la fenêtre de la saison courante), **0 ensuite**. Évite que le fondateur reparte Seigneur avec +200 à chaque nouvelle Coupe (privilège permanent = anti-méritocratie). **Doit s'appliquer aussi à `_faction_chef` (mig 302, prod)** puisque Seigneur = Chef. Couronnes **conquises** (÷10) restent permanentes (résiduel mineur, pas de donnée par saison — à revisiter si besoin). |
