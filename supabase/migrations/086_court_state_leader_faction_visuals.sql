-- 086_court_state_leader_faction_visuals.sql
-- WHY : la fiche de lieu doit centrer le veilleur sur la PERSONNE (le leader
-- de l'expédition), pas sur la faction. Pour ça on expose dans le retour de
-- get_place_court_state :
--   - veilleur.leaderName : prénom/nom du membre fondateur de l'expédition
--   - veilleur.factionColor : couleur de sa faction
--   - veilleur.factionPattern : URL du pattern SVG (icône faction)
-- Le frontend affiche "Lieu protégé par [leader]" + une boule colorée (color)
-- avec l'icône (pattern en mask-image).
...
BEGIN
  ...
  -- Veilleur object — V086 : ajout leaderName + factionColor + factionPattern
  -- Le leader = premier membre joint à l'expédition (ordre stable par user_id).
  ...
  LEFT JOIN LATERAL (
    SELECT u.id, u.display_name, u.first_name, u.email_address
    FROM public.expedition_members em
    JOIN public.users u ON u.id = em.user_id
    WHERE em.expedition_id = e.id
    ORDER BY u.id
