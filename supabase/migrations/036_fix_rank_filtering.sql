-- 036_fix_rank_filtering.sql
-- WHY : bug dans la mig 035 — le WHERE id = p_user_id filtrait AVANT le
-- ROW_NUMBER(). PostgreSQL applique WHERE avant les window functions, donc
-- ROW_NUMBER était calculé sur 1 seule ligne et retournait toujours 1.
-- Conséquence : tout le monde avait rank=1 dès qu'il avait un score, donc
-- tout le monde décrochait le titre le plus haut (Prélat ou équivalent).
--
-- Fix : calculer ROW_NUMBER en sous-requête sur TOUS les users de la faction,
-- PUIS filtrer le résultat sur le user concerné.
--
-- Reprise EXACTE de la mig 035, seul le bloc de calcul du rank change.
...
BEGIN
  ...
  IF v_faction_id IS NOT NULL THEN
    ...
    -- ROW_NUMBER calculé sur TOUS les users de la faction d'abord, puis on
    -- filtre le résultat. SI on faisait WHERE id = p_user_id avant, PostgreSQL
    -- applique WHERE avant la window function → ROW_NUMBER sur 1 seule ligne
    -- = toujours 1. Bug de la mig 035.
