-- 079_rating_on_contributions.sql
-- Ajouter le rating du joueur sur chaque contribution (pour afficher les étoiles sur les carnets)
...
BEGIN
  ...
  -- Contributions (triées par votes) — LEFT JOIN place_ratings pour les étoiles
  SELECT json_agg(
    json_build_object(
      'id', pc.id,
      'userId', pc.user_id,
