...
- V0.10.5 (25 juin 2026) : **Compagnie PRINCIPALE + ALLIÉE** (mig 295). Un joueur a 1 Compagnie principale (`users.faction_id` = points/territoires/Chef/grades) et au plus 1 Alliée (2e adhésion `faction_members` ≠ `faction_id` : chat + badge « Allié », **0 point**, exclue Chef/classement — sa Coupe est déjà 0 via banner-history). Le terme « officielle » est abandonné (collision avec `isOfficial` = Compagnie historique RdC vs créée). RPC neuve `set_primary_faction` (bascule délibérée : cooldown `faction_change_cooldown_days`=30 + **repaint contrôlé** des veilles non-neutres du joueur) **remplace** le toggle casual `set_active_faction` (laissé vivant en prod, à DROP au cleanup une fois le front déployé). `_faction_chef` filtre `u.faction_id = p_faction_id` (l'allié ne règne jamais, même riche en couronnes). `get_faction_detail` expose `isAlly` par membre. Front : store `factionGroupStore.setPrimary` (ex-`switchBanner`), `FactionMember.isAlly`. `BannerToggle` n'a plus de cycle casual (tap → ouvre le Hall de la principale). Action « Désigner comme principale » (confirm + cooldown) dans `FactionHallModal` (footer membre), badge « Allié » dans Hall/CompaniesPage/FactionModal. Spec : `docs/superpowers/specs/2026-06-24-compagnie-officielle-et-allie.md`.
...
- V0.8.23 (24 mai 2026) : **mécénat d'un challenger** (soutenir un attaquant), geste **tap** (1 clic = 1 Couronne, burst + score optimiste), PAS de modale. La Cour est user-centric (veilleur = user, `place_court_action.beneficiary_user_id`). `invest_crowns` reçoit un param optionnel `p_beneficiary_user_id` (mig 173) : crédite le score d'un challenger désigné au lieu du caller, avec dérivation serveur de son expé (anti `place_veille` incohérent) + erreurs `not_a_challenger`/`challenger_expedition_missing`. `get_place_court_state.challengers[]` expose le score-bénéficiaire + `supporters[]` (mécènes du challenger hors lui-même, mig 174). Front : bouton tap « Soutenir » par challenger dans `PatronsList` + sous-liste « ↳ Soutiens » (symétrie veilleur) ; `PlaceCourtView` étend le tap-burst (`pushTap` + `PendingTaps.beneficiaryUserId` + `BurstAnim.key`), optimiste par challenger. `InvestCrownsModal` **supprimée** (orpheline, jamais le bon pattern). Notif au challenger soutenu via `place_court_support` + `targetSide:'attack'`. Faiseur de roi : le soutenu prend le trône. Spec/plan : `docs/superpowers/specs|plans/2026-05-24-mecenat-challenger*`.
