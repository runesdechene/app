-- 150_invest_crowns_preserve_invests_on_basculement.sql
-- WHY : game-design (Uriel 9/05). Aujourd'hui une bascule par influence
-- (challenger qui dépasse la défense effective via invest_crowns) déclenche
--   DELETE FROM place_court_score WHERE place_id = X AND expedition_id != gagnant
-- Ce wipe efface les Couronnes investies par tous les autres patrons sur les
-- autres expéditions du lieu (l'ancien défenseur + les autres challengers).
--
-- Décision Uriel : le wipe des invests des autres ne doit avoir lieu que lors
-- d'un plant_flag GPS (cas A/B/C/D). Une bascule par influence change juste
-- "qui occupe le lieu" — pas une remise à zéro de l'engagement Cour de tout
-- le monde. Conséquence voulue : après bascule, l'ancien défenseur devient
-- challenger avec son score d'invest préservé, les autres challengers gardent
-- aussi leur score. Le nouveau veilleur hérite donc immédiatement d'un
-- contexte tendu — cohérent avec la dynamique Cour fluide.
--
-- Reprise verbatim de la mig 138, seul change : suppression du DELETE
-- post-bascule (lignes 142-144 de la mig 138).
  ...
  IF v_side = 'attack' THEN
    ...
    ELSE
      ...
      IF v_new_target_score > v_veilleur_score THEN
        ...
        -- V150 : on ne wipe PLUS les place_court_score des autres expéditions
        -- lors d'une bascule par influence. Les invests des autres patrons
        -- sont préservés. Le wipe est réservé au plant_flag GPS.
