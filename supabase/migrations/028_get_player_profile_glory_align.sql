-- 028_get_player_profile_glory_align.sql
-- WHY : Bug d'incohérence — `get_player_profile.glory` retournait l'ancienne
-- formule (exploration_points + erudition_points) alors que le LeaderboardModal
-- et le NotorietyBadge utilisent la nouvelle Gloire à la volée (mig 024+026+027).
-- Conséquence : un user voyait 6200 dans le classement, mais 219 sur son
-- profil — incompatible.
--
-- Fix : aligner get_player_profile.glory sur la formule unifiée.
-- Reprise EXACTE de la version mig 011 (procédure "lire avant de réécrire"
-- du gotchas.md), seul le calcul de glory change (ligne 168 de la mig 011).
...
BEGIN
  ...
  -- V0.7 phase 3.5 — Gloire calculée à la volée (formule unifiée avec
  -- get_my_glory et get_leaderboard). Avant : exploration_points + erudition_points.
  v_glory :=
      COALESCE((SELECT COUNT(DISTINCT place_id) FROM public.place_explorers WHERE user_id = p_user_id), 0) * 1
    ...
    + COALESCE((SELECT COUNT(*)            FROM public.enigma_responses   WHERE user_id = p_user_id AND correct = TRUE), 0) * 1;
