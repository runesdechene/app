-- 334_mod_list_places_masked_exclusive.sql
-- WHY : décision Uriel « masqué = masqué ». Un lieu masqué ne doit plus polluer la
-- file « À traiter » (ni « Vérifiés ») : il vit uniquement dans la catégorie
-- « Masqués » (et « Tous »). Les branches 'unverified' et 'verified' excluent
-- désormais masked=true → les 3 catégories deviennent mutuellement exclusives
-- (masked + unverified-non-masqué + verified-non-masqué = total).
-- Corps copié de la def LIVE (mig 333), seules les 2 branches modifiées.
--
-- Réversible : restaurer le corps mig 333.
...
BEGIN
  ...
  SELECT count(*) INTO v_total
   ...
   WHERE (p_search IS NULL OR p.title ILIKE '%'||p_search||'%')
     AND (p_filter <> 'unverified' OR (p.verified_at IS NULL     AND p.masked = false))
     AND (p_filter <> 'verified'   OR (p.verified_at IS NOT NULL AND p.masked = false))
