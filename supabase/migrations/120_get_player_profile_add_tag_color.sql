-- 120_get_player_profile_add_tag_color.sql
-- WHY : tagIcon (mig 118) est rendu dans une "petite boule colorée" comme
-- dans TagBonusList et l'encart affinités du profil — il faut donc aussi
-- la couleur du tag primary pour le fond du cercle. On ajoute 'tagColor'
-- au JSON des 4 carrousels (authored / discovered / favorite / veilled).
--
-- Reprise EXACTE de la mig 118 (procédure "copier la baseline entière")
-- — seuls 4 endroits changent : ajout de 'tagColor', primary_tag.color
-- dans chaque json_build_object des sous-requêtes places.
