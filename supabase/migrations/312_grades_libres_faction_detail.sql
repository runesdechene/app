-- 312_grades_libres_faction_detail.sql
-- WHY : grade par membre via parcours de capacités (inline, depuis principal_pos — pas d'appel
-- _member_grade_rank par membre = O(N²) sur _user_faction_coupe). + expose la structure `grades`
-- (libellés + capacités, catch-all en dernier) et `governGrades`. Corps = mig 307 + gbounds + 2 clés.
...
BEGIN
  ...
  gbounds AS (
    -- bornes cumulées : grade(position P) = MIN(rank) où cum_upper >= P. Catch-all (capacity NULL) = ∞.
