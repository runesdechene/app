-- 174_court_challenger_supporters.sql
-- WHY : UX mécénat challenger (Uriel 24/05). Le soutien à un attaquant se fait
-- en tap (1 clic = 1 Couronne), et chaque challenger affiche en sous-liste ses
-- mécènes (« ↳ Soutiens »), symétrie avec le veilleur. On ajoute donc, dans le
-- champ `challengers` de get_place_court_state, la liste `supporters` =
-- les users qui financent ce challenger (hors le challenger lui-même).
--
-- Baseline : get_place_court_state mig 173 (verbatim), seul ajout = le champ
-- 'supporters' dans le jsonb_build_object des challengers.
