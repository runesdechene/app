-- 249_court_state_member_is_expedition.sql
-- WHY : isMemberOfVeilleur ne testait que le LEAD (p_user_id = veilleur_user_id),
-- donc les co-veilleurs (membres de l'expédition, hors lead) étaient vus comme
-- non-membres → ils pouvaient « trahir » et influencer contre leur propre compagnie.
-- Fix : v_is_member_v teste l'appartenance à l'EXPÉDITION veilleuse (expedition_members).
-- Discipline B1 : copie verbatim du live + cette seule ligne changée.

CREATE OR REPLACE FUNCTION public.get_place_court_state(p_place_id text, p_user_id text DEFAULT NULL::text)
 RETURNS json
 LANGUAGE plpgsql
...
BEGIN
  ...
  -- V173/V174 : challengers user-centric = cibles soutenables, + leurs mécènes.
  WITH chal AS (
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    ...
    -- V174 : mécènes de ce challenger (hors lui-même), triés par montant.
    'supporters', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
        'userId',      s.user_id,
        'displayName', COALESCE(su.display_name, su.first_name, su.id),
        'amount',      s.amount
      ) ORDER BY s.amount DESC)
      FROM (
        SELECT pca2.user_id, SUM(pca2.amount)::integer AS amount
        FROM public.place_court_action pca2
        WHERE pca2.place_id = p_place_id
          AND pca2.beneficiary_user_id = c.user_id
          AND pca2.user_id <> c.user_id
        GROUP BY pca2.user_id
        HAVING SUM(pca2.amount) > 0
      ) s
  ...
  -- V249 : membre = appartenir à l'EXPÉDITION veilleuse (pas seulement être le lead).
  v_is_member_v := (NOT v_vacant AND EXISTS (
