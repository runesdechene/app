...
|---|---|
| `supabase/migrations/040_v07_levels_schema.sql` | ALTER TABLE users (xp_total, veteran_first_era), app_settings xp_epoch, function `_level_from_xp` |
| `supabase/migrations/042_v07_levels_triggers.sql` | Triggers INSERT/DELETE sur 5 tables d'action |
| `supabase/migrations/043_v07_levels_titles_refonte.sql` | DELETE Bâtisseur + UPDATE titres existants + INSERT nouveaux (33 finaux) |
| `supabase/migrations/044_v07_levels_get_user_titles.sql` | Refonte `get_user_titles` pour évaluer sur compteurs étendus |
| `supabase/migrations/045_v07_levels_player_profile.sql` | Refonte `get_player_profile` (level/xp_total/xpToNextLevel/veteran_first_era, plus de glory) |
| `supabase/migrations/046_v07_levels_leaderboard.sql` | Refonte `get_leaderboard('notoriety')` tri par level |
| `supabase/migrations/047_v07_levels_action_rpcs.sql` | Enrichissement des RPCs d'action (xp_delta, level_before, level_after) + gating cartographier niveau 3 |
| `supabase/migrations/048_v07_levels_veteran_marking.sql` | UPDATE veteran_first_era pour les contributeurs historiques (à appliquer juste avant déploiement front) |
| `supabase/migrations/049_v07_drop_glory_legacy.sql` | **Différée ~2 semaines post-déploiement.** DROP `get_my_glory`, colonnes `exploration_points`/`erudition_points` |
...
--   - Stocke xp_epoch dans app_settings (date du switch, secondes UTC)
...
```sql
SELECT key, value FROM public.app_settings WHERE key = 'xp_epoch';
```
...
### Task 2: Migration triggers `042_v07_levels_triggers.sql`

**Files:**
- Create: `supabase/migrations/042_v07_levels_triggers.sql`
...
```bash
git add supabase/migrations/042_v07_levels_triggers.sql
...
### Task 3: Migration titres `043_v07_levels_titles_refonte.sql`

**Files:**
- Create: `supabase/migrations/043_v07_levels_titles_refonte.sql`
...
```bash
git add supabase/migrations/043_v07_levels_titles_refonte.sql
...
### Task 4: Migration `044_v07_levels_get_user_titles.sql` — refonte de la fonction d'évaluation

**Files:**
- Create: `supabase/migrations/044_v07_levels_get_user_titles.sql`
...
```bash
git add supabase/migrations/044_v07_levels_get_user_titles.sql
...
### Task 5: Migration `045_v07_levels_player_profile.sql` — refonte get_player_profile

**Files:**
- Create: `supabase/migrations/045_v07_levels_player_profile.sql`
...
```bash
git add supabase/migrations/045_v07_levels_player_profile.sql
...
### Task 6: Migration `046_v07_levels_leaderboard.sql` — refonte get_leaderboard notoriety

**Files:**
- Create: `supabase/migrations/046_v07_levels_leaderboard.sql`
...
```bash
git add supabase/migrations/046_v07_levels_leaderboard.sql
...
### Task 7: Migration `047_v07_levels_action_rpcs.sql` — RPCs d'action enrichies + gating cartographier

**Files:**
- Create: `supabase/migrations/047_v07_levels_action_rpcs.sql`

- [ ] **Step 1: Créer le fichier (helper de réponse + wrapper sur add_place)**

```sql
-- 047_v07_levels_action_rpcs.sql
...
```bash
git add supabase/migrations/047_v07_levels_action_rpcs.sql
...
### Task 21: Migration `048_v07_levels_veteran_marking.sql`

**Files:**
- Create: `supabase/migrations/048_v07_levels_veteran_marking.sql`
...
```sql
-- 048_v07_levels_veteran_marking.sql
...
```bash
git add supabase/migrations/048_v07_levels_veteran_marking.sql
...
### Task 24: Migration différée `049_v07_drop_glory_legacy.sql`

**Files:**
- Create: `supabase/migrations/049_v07_drop_glory_legacy.sql`
...
```bash
git add supabase/migrations/049_v07_drop_glory_legacy.sql
