-- 309_plant_flag_recolor_to_current_faction.sql
-- WHY : bug « je capture un lieu mais il reste à l'ancienne faction » (rapport Néphérys, Château de
-- Lourmarin resté vert/celtique alors qu'elle est au Lys de Fer/bleu). Après le chamboulement des
-- factions V0.10 (anciennes retirées, joueurs changeant de Compagnie), les cas réaffirmer/confirmer/
-- reconquérir de plant_flag (CAS A/B/D) RÉUTILISENT la vieille expédition et GARDENT sa couleur figée
-- au lieu de recolorer à la faction ACTUELLE du veilleur. Décision Uriel 26/06 : recapturer recolore
-- toujours à la faction actuelle. Fix : les 3 cas posent désormais place_veille.faction_id = la faction
-- du capteur (v_user_faction, non-neutre) + recolorent l'expédition réutilisée. Corps = mig 300 verbatim,
-- seuls ces 3 UPDATE changent (+ une recolorisation d'expédition par cas). ADDITIF.

CREATE OR REPLACE FUNCTION public.plant_flag(p_user_id text, p_place_id text, p_user_lat numeric, p_user_lng numeric, p_partners_user_ids text[] DEFAULT '{}'::text[], p_expedition_name text DEFAULT NULL::text)
 RETURNS json
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
DECLARE
  ...
  v_pv_faction text; v_pv_neutral boolean;
BEGIN
  ...
  THEN
    -- recolor : la faction du capteur prime sur la faction figée de l'expédition réutilisée
    UPDATE public.expeditions SET faction_id = v_user_faction, is_neutral = false WHERE id = v_prev_previous_exp;
    ...
    SELECT faction_id, is_neutral INTO v_pv_faction, v_pv_neutral FROM public.place_veille WHERE place_id = p_place_id;
    RETURN json_build_object('success', true, 'mode', 'reclaim_gps', 'placeId', p_place_id,
      ...
      'plantBonus', v_plant_bonus_amount, 'factionId', v_pv_faction, 'isNeutral', v_pv_neutral);
  END IF;
  ...
  THEN
    -- recolor : faction du capteur
    UPDATE public.expeditions SET faction_id = v_user_faction, is_neutral = false WHERE id = v_prev_veilleur_exp;
    UPDATE public.place_veille SET by_influence = false, previous_expedition_id = NULL, planted_at = v_now, veilleur_user_id = p_user_id,
        faction_id = v_user_faction, is_neutral = false
    WHERE place_id = p_place_id;
  ...
  IF COALESCE(v_prev_by_influence, false) = false
     ...
     AND EXISTS (SELECT 1 FROM public.expedition_members em WHERE em.expedition_id = v_prev_veilleur_exp AND em.user_id = p_user_id)
  THEN
    -- recolor : faction du capteur (réaffirmer un lieu le repasse à TA couleur actuelle)
    UPDATE public.expeditions SET faction_id = v_user_faction, is_neutral = false WHERE id = v_prev_veilleur_exp;
    UPDATE public.place_veille SET planted_at = v_now, veilleur_user_id = p_user_id,
        faction_id = v_user_faction, is_neutral = false
    WHERE place_id = p_place_id;
    ...
    RETURN json_build_object('success', true, 'mode', 'reaffirm_gps', 'placeId', p_place_id,
      ...
      'factionId', v_pv_faction, 'isNeutral', v_pv_neutral);
  END IF;
  ...
  BEGIN
    ...
    v_title := CASE WHEN COALESCE(array_length(p_partners_user_ids, 1), 0) > 0
      THEN COALESCE(NULLIF(btrim(p_expedition_name), ''), 'Expédition de ' || v_planter_name)
      ELSE NULL END;
