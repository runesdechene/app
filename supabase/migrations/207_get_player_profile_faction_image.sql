-- 207_get_player_profile_faction_image.sql
-- WHY : le profil joueur affichait l'emblème teinté (factions.pattern). On bascule
-- vers la vraie bannière illustrée (factions.image_url) posée à gauche du nom. La
-- RPC ne renvoyait pas image_url — on l'expose sous 'factionImage'. Le front
-- garde un fallback sur factionPattern tant qu'une faction n'a pas de bannière.
-- Discipline B1 : copie verbatim de la baseline (mig 158) + 1 ajout ciblé (json output).
