-- 096_plant_flag_reaffirm_irl.sql
-- WHY : un plein-veilleur qui revient sur son lieu IRL doit pouvoir
-- réaffirmer son contrôle — i.e. effacer les menaces challengers en cours.
-- C'est le pendant du cas A (reclaim by_influence) pour les plein-veilleurs.
--
-- Avant cette mig (mig 092), Diane qui revenait sur son propre lieu se
-- retrouvait bloquée par 'already_yours' et devait défendre Couronne par
-- Couronne. Pas cohérent avec "la marche prime sur l'or".
--
-- Après cette mig (cas D' = "Reaffirm IRL") :
--   Condition : by_influence=false ET planteur ∈ expé veilleuse plein.
--   Action :
--     - DELETE place_court_score WHERE expedition_id != veilleur (efface
--       UNIQUEMENT les challengers ; la défense propre du veilleur est
--       conservée — toutes les Couronnes investies en défense sont gardées)
--     - UPDATE place_veille SET planted_at = now() (refresh)
--     - Notif activity_log type='place_reaffirmed' (pour les challengers déchus)
--     - Pas de cooldown 24h (revisiter son propre lieu doit être fluide)
--   Retour mode : 'reaffirm_gps'
...
BEGIN
  ...
  -- État : by_influence = false ET planteur ∈ expé veilleuse plein.
  -- Action : reset UNIQUEMENT les challengers, conserve la défense propre,
  -- pas de cooldown, refresh planted_at.
  -- ============================================================
  ...
  THEN
    ...
    -- Reset des challengers SEULEMENT (la défense de l'expé veilleuse reste)
    ...
    -- Notif si effectivement des menaces ont été nettoyées
    IF v_threats_cleared > 0 THEN
      INSERT INTO public.activity_log (type, actor_id, place_id, data)
      VALUES ('place_reaffirmed', p_user_id, p_place_id, jsonb_build_object(
        'placeTitle',     v_place_title,
        'expeditionId',   v_prev_veilleur,
