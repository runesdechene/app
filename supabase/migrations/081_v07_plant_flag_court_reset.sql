-- 081_v07_plant_flag_court_reset.sql
-- WHY : Phase 5 — plant_flag doit gérer le reset des scores de la Cour et
-- les transitions du flag by_influence (réversibilité du veilleur "par influence").
--
-- Trois cas distincts au plantage :
--
-- CAS A — Reclaim par ancien veilleur déchu (la marche prime sur l'or) :
--   Le lieu est dans l'état by_influence=true avec previous_expedition_id=X,
--   et le user qui plante appartient à l'expé X. → Reprise gratuite : on remet
--   place_veille.expedition_id ← X, by_influence ← false, previous ← NULL.
--   Pas de cooldown (la marche prime). Notif aux déchus (place_taken_back_gps).
--
-- CAS B — Confirmation IRL par l'expé "par influence" :
--   Le lieu est dans l'état by_influence=true, expedition_id=Y, et le user
--   appartient à Y (mais pas à previous_expedition_id). → Confirmation :
--   by_influence ← false, previous ← NULL. Pas de cooldown. Pas de notif
--   spéciale (le toast plant_flag standard suffira si on en log un).
--
-- CAS C — Plantage standard (comportement mig 067) :
--   Tout autre cas. On crée une nouvelle expédition, supplante via UPSERT
--   place_veille. En plus : DELETE court_score, reset by_influence/previous.
--   Cooldown 24h appliqué.
--
-- Reécriture verbatim mig 067 plant_flag + ajout des cas A/B en début + reset
-- court_score dans le cas C + UPSERT enrichi des nouvelles colonnes.
...
DECLARE
  ...
  v_caller_in_prev      boolean;
  v_caller_in_curr      boolean;
BEGIN
  ...
  -- ============================================================
  -- V081 — Lire l'état actuel pour décider du cas
  -- ============================================================
  ...
  THEN
    -- Pas de cooldown pour reclaim (la marche prime).
    -- Mettre à jour place_veille en remettant l'ancien
    ...
    -- Reset des scores Cour
    DELETE FROM public.place_court_score WHERE place_id = p_place_id;

    -- Notif aux membres de l'expé déchue (= v_prev_veilleur, qui détenait par influence)
    ...
    -- Log historique : 1 ligne pour le caller (continuité veille_history)
    ...
    -- Members JSON de l'expé reclaim
  ...
  THEN
    -- Pas de cooldown : confirmation gratuite IRL
    UPDATE public.place_veille
  ...
  -- ============================================================
  -- CAS C — Plantage standard (verbatim mig 067)
  -- ============================================================

  -- Cooldown anti-farm collusion
  ...
  -- V081 : UPSERT place_veille avec reset by_influence + previous_expedition_id
  -- (le nouveau veilleur démarre frais, faveur 50 implicite)
  ...
  -- V081 : reset des scores Cour (nouveau veilleur, page blanche)
  DELETE FROM public.place_court_score WHERE place_id = p_place_id;
