-- 283_coupe_state_personal_gold.sql
-- WHY : la Coupe d'une Compagnie inclut l'or conquis par ses membres (mig 277-280),
-- mais le bloc personnel du CoupeModal (topUsers + myBreakdown) ne comptait que les
-- ACTIONS → un membre qui n'a que de l'or voyait sa contribution à 0. On intègre l'or
-- perso (member-gated, même tranche/cap) à la contribution personnelle, pour la cohérence
-- avec le Hall. L'agrégation par Compagnie (faction_combined) reste inchangée (l'or y est
-- déjà ajouté séparément → pas de double comptage). ADDITIF.

-- Or-Coupe personnel d'un joueur (toutes ses Compagnies, member-gated, day-capped).
CREATE OR REPLACE FUNCTION public._user_gold_coupe(
  p_user_id text, p_from timestamptz DEFAULT NULL, p_to timestamptz DEFAULT NULL
...
$$;
GRANT EXECUTE ON FUNCTION public._user_gold_coupe(text,timestamptz,timestamptz) TO authenticated, anon, service_role;
...
DECLARE
  ...
  v_gtranche     int := GREATEST(_barem('coupe.gold_per_tranche', 10), 1);
  v_gcap         int := _barem('coupe.gold_daily_cap', 10);
BEGIN
  ...
  -- ── Agrégation par Compagnie (action membres + or Compagnie) — inchangée ──
  WITH user_scores AS (
  ...
  faction_action AS (
    ...
    JOIN public.faction_members fm
      ON fm.user_id = uwm.user_id AND fm.faction_id = uwm.faction_id
  ...
  -- ── Top contributeurs (personnel) = action + OR perso ──
  WITH user_scores AS (
    SELECT pd.user_id, COUNT(*)::int * v_c_discover AS score
    FROM public.places_discovered pd
    WHERE pd.discovered_at >= v_season.started_at AND pd.discovered_at < v_window_end
    ...
    GROUP BY er.user_id
    UNION ALL
    -- OR perso (member-gated, tranche + cap/jour)
    SELECT g.user_id,
           SUM(LEAST(g.amount / v_gtranche, v_gcap))::int
    FROM public.faction_gold_log g
    JOIN public.faction_members fm ON fm.user_id = g.user_id AND fm.faction_id = g.faction_id
    WHERE g.day >= v_season.started_at::date AND g.day <= v_window_end::date
    GROUP BY g.user_id
  )
  SELECT jsonb_agg(jsonb_build_object(
    'userId',      tu.user_id,
  ...
  -- ── Ma contribution (personnel) = action + OR perso ──
  IF p_user_id IS NOT NULL THEN
    SELECT jsonb_build_object(
      ...
      'score', public._user_coupe_score(p_user_id, v_season.started_at, v_window_end)
               + public._user_gold_coupe(p_user_id, v_season.started_at, v_window_end)
    )
    INTO v_my_breakdown
    FROM
