-- 025_fix_photos_count.sql
-- WHY : bug du compteur de photos dans get_my_glory + get_coupe_state.
--
-- Avant : COUNT(*) WHERE type='photo' sur place_contributions → comptait le
-- nombre de LIGNES type='photo'. Or les photos sont stockées dans le champ
-- images JSONB (array), avec souvent plusieurs photos par ligne — notamment
-- le carnet auto-créé à l'ajout d'un lieu (mig 005) qui agrège les photos
-- initiales du lieu en jsonb_agg(images) dans place_contributions.images.
--
-- Effet : un user qui crée un lieu avec 5 photos voyait son compteur photos
-- à 0 (car les photos sont dans la row type='carnet' du carnet auto, pas
-- dans une row type='photo' séparée).
--
-- Fix : compter SUM(jsonb_array_length(images)) sur TOUTES les contributions
-- du user, peu importe le type. Ça inclut les photos initiales (via carnet
-- auto) ET les photos ajoutées après (contributions explicites). Plus on
-- ajoute le case `image_url` non vide pour les vieilles rows pré-mig-005
-- (rétrocompat).
...
BEGIN
  ...
  -- Carnets : 1 ligne = 1 carnet (peu importe combien de photos il contient)
  SELECT COUNT(*) INTO v_carnets
  FROM public.place_contributions
  WHERE user_id = p_user_id AND type = 'carnet';

  -- Photos : on compte le NOMBRE TOTAL d'images (jsonb_array_length) sur TOUTES
  -- les contributions du user, peu importe le type. Plus une fallback +1 pour
  -- les vieilles rows pré-mig-005 où images est vide mais image_url renseigné.
  SELECT COALESCE(SUM(
...
-- Idem pour get_coupe_state — sources et myBreakdown corrigés sur les photos.
-- (Reprise EXACTE de la version 024 mais avec la nouvelle formule de comptage
-- pour les rows photo dans les CTE et myBreakdown.)
...
BEGIN
  ...
  WITH user_scores AS (
    ...
    GROUP BY u.id
    UNION ALL
    -- Photos : SUM(jsonb_array_length(images)) sur TOUTES les contributions
