-- 247_plant_flag_expedition_name.sql
-- WHY : le plantage en commun permet désormais de NOMMER l'expédition (UX V0.9.55+).
-- plant_flag reçoit un param optionnel p_expedition_name : si fourni (et plantage avec
-- compagnons → CAS C), il devient le titre de l'expédition au lieu de l'auto
-- « Expédition de {nom} ». Les autres cas (reclaim/confirm/reaffirm) ne créent pas
-- d'expédition, donc non concernés.
-- Discipline B1 : copie verbatim de plant_flag (mig 246) + 6e param + 1 ligne de titre.
-- Signature changée (5→6 args) → DROP de l'ancienne pour éviter l'overload ambigu.

DROP FUNCTION IF EXISTS public.plant_flag(text, text, numeric, numeric, text[]);

CREATE OR REPLACE FUNCTION public.plant_flag(p_user_id text, p_place_id text, p_user_lat numeric, p_user_lng numeric, p_partners_user_ids text[] DEFAULT '{}'::text[], p_expedition_name text DEFAULT NULL)
 RETURNS json
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
...
BEGIN
  ...
  -- partenaires présents sont marqués explorateurs du lieu.
  ...
  BEGIN
    ...
    -- V247 : nom d'expédition custom si fourni (plantage avec compagnons), sinon auto.
...
GRANT EXECUTE ON FUNCTION public.plant_flag(text, text, numeric, numeric, text[], text) TO authenticated, service_role;
