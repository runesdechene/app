-- 296_player_profile_ally_faction.sql
-- WHY : le profil n'exposait que la Compagnie PRINCIPALE (users.faction_id). On ajoute la
-- Compagnie ALLIÉE (2e adhésion non-retirée ≠ principale) → 4 champs allyFaction* pour
-- afficher « Allié de la Compagnie X » sous l'appartenance principale, sur tout profil.
-- Corps = état prod de get_player_profile + 1 LEFT JOIN LATERAL `ally` + 4 clés JSON.
-- ADDITIF (redéfinition backward-compatible : que des champs en plus).
...
BEGIN
  ...
  SELECT json_build_object(
    ...
    -- Compagnie ALLIÉE (2e adhésion non-retirée ≠ principale) — null si aucune.
